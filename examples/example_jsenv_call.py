...
from jsenv.nodeenv.execjs import ExecJS, Partial
from jsenv.nodeenv.jsdom import JSDOM
...
# 同步模式
def base_exec_call(script, name, *args, **kwargs):
    """基础 ExecJS 环境"""
    asis = ExecJS()
    asis.setup.append(script)
    return asis(Partial(name, *args, **kwargs))
...
def base_jsdom_call(
    ...
    **kwargs
    ...
    dom = JSDOM(src=src, ua=ua, location=location, referrer=referrer)
    dom.setup.append(script)
    return dom(Partial(name, *args, **kwargs))


def my_jsdom_call(script, name, *args, **kwargs):
    """自定义 DOM 环境"""
    ua = "Mozilla/5.0 (Linux; Android 11; Pixel 5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.91 Mobile Safari/537.36"
    location = "http://httpbin.org"
    referrer = "http://httpbin.org"
    src = ""
    return base_jsdom_call(
        script,
        name,
        src=src,
        ua=ua,
...
if __name__ == "__main__":
    ...
    output = base_exec_call("function a(){return Math.random();}", "a")
    print(output)
    output = base_exec_call("function a(i){return i;}", "a", '1')
    print(output)
    output = base_jsdom_call("function a(){return Math.random();}", "a")
    print(output)
    output = my_jsdom_call("function a(){return Math.random();}", "a")
    print(output)